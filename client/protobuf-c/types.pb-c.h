...
typedef struct _XmrSignatureType XmrSignatureType;
...
typedef enum _ButtonRequestType {
  ...
  BUTTON_REQUEST_TYPE__ButtonRequest_PublicKey = 11
...
struct  _CoinType
{
  ...
  protobuf_c_boolean has_address_type_p2wpkh;
  uint32_t address_type_p2wpkh;
  protobuf_c_boolean has_address_type_p2wsh;
  uint32_t address_type_p2wsh;
  char *signed_message_header;
};
#define COIN_TYPE__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&coin_type__descriptor) \
    , NULL, NULL, 0,0u, 0,0, 0,5u, 0,6u, 0,10u, NULL }
...
struct  _XmrInputType
{
  ...
  /*
   * relative key offsets (limit to MIXIN: 10)
   */
...
/*
 **
 * Structure representing SIG data
 * @used XmrGenerateSignature
 */
struct  _XmrSignatureType
{
  ProtobufCMessage base;
  /*
   * key derivation
  ...
  /*
   * signature data (limit to MIXIN: 10 or 672 bytes)
   */
  protobuf_c_boolean has_stream;
  ProtobufCBinaryData stream;
  /*
   * sum
   */
  ProtobufCBinaryData sum;
};
#define XMR_SIGNATURE_TYPE__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&xmr_signature_type__descriptor) \
...
typedef void (*XmrInputType_Closure)
                 (const XmrInputType *message,
                  void *closure_data);
typedef void (*XmrSignatureType_Closure)
                 (const XmrSignatureType *message,
                  void *closure_data);
...
extern const ProtobufCMessageDescriptor xmr_signature_type__descriptor;
